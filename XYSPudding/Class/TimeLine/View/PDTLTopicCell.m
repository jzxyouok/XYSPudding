                       ...
                       index:(NSInteger)index_
{
    ...
    [self.avatar setCircleAvatarWithURL:[_viewModel avatarURLWithIndex:_index_]
                       placeholderImage:[UIImage imageNamed:@"default_avatar_64x64_"]];
    [self.nickNameLabel setText:[_viewModel nickNameWithIndex:_index_]];
    [self.presentTimeLabel setText:[_viewModel presentTimeWithIndex:_index_]];
    self.concernButton.tag = 0;
    [self.contentLabel setText:[_viewModel contentWithIndex:_index_]];
    
    if ([_viewModel isContainImageWithIndex:_index_])
    {
        NSInteger count = [_viewModel imageNumberWithIndex:_index_];
        ...
        for (NSInteger i = 0; i < count; i++)
        {
            ...
            [imageView setImageWithURL:[_viewModel imageURLWithRowIndex:_index_
                                                             imageIndex:i]
    ...
    if ([_viewModel repostCountWithIndex:_index_] != 0)
    {
        [self.repostButton normalTitle:[NSString stringWithFormat:@"%@", @([_viewModel repostCountWithIndex:_index_])]];
    }
    else
    {
        [self.repostButton normalTitle:@"转发"];
    }
    if ([_viewModel replyCountWithIndex:_index_] != 0)
    {
        [self.replyButton normalTitle:[NSString stringWithFormat:@"%@", @([_viewModel replyCountWithIndex:_index_])]];
    }
    else
    {
        [self.repostButton normalTitle:@"回复"];
    }
    if ([_viewModel praiseCountWithIndex:_index_] != 0)
    {
        [self.praiseButton normalTitle:[NSString stringWithFormat:@"%@", @([_viewModel praiseCountWithIndex:_index_])]];
    }
    else
    {
